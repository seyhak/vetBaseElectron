...
export const useCatalogue = () => {
  ...
  const loadDetailedItem = useCallback(
    async (id: string) => {
      const loadedItem = await (window as any).electronAPI.getDetailedItem(id)
  ...
  useEffect(() => {
    // console.log(location, Object.fromEntries(new URLSearchParams(location.search)))
  ...
  const editItem = useCallback(
    async (id: string, title: string, description: Descendant[]) => {
